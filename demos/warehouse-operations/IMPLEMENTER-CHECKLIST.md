...
| NVIDIA access | An NGC API key that can read the protected repositories; revalidate access for the implementer's organization |
...
| 3. Verify NVIDIA inputs | NGC API key and required entitlements | Run `./demos/warehouse-operations/scripts/resolve-protected-ngc-images.sh`; compare the returned digests with the [source lock](openshift/nvidia-warehouse/source-lock.yaml) and chart values | Revalidated immutable digest record for every protected NVIDIA image | Alert Verification, Configurator, RTVI-VLM, and Nemotron Nano match the checked-in digests; no `latest` or unresolved external tag remains |
...
|---|---|---|
| Derived application images remain unresolved | Build and pin the source-defined Elasticsearch and helper/init images before completing the full render | [WAREHOUSE-CONFIG-INVENTORY.md](research/WAREHOUSE-CONFIG-INVENTORY.md) and [chart README](openshift/nvidia-warehouse/chart/README.md) |
